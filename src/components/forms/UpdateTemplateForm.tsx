...
const UpdateTemplateForm = ({ isOpen, setOpen, target }: Props) => {
  ...
  const formStructure = useMemo(() => {
    return target
          ...
          update_template: {
            entries: {
              ...
              time_limit: {
                ...
                validate: (data) => {
                  return Number(data.update_template.time_limit) > 0
                    ? { ok: true }
      ...
      : {};
  }, [target, templates, validName]);

  const onEnter = useCallback(
    async ({ update_template: data }) => {
      const { name, description, allow_notification, time_limit, is_exam } =
        data;
      await updateTemplateInfo(
        target.id,
        name,
        description,
        is_exam,
...
export default React.memo(UpdateTemplateForm, (p, n) => {
  return (
    p.isOpen == n.isOpen &&
    p.setOpen == n.setOpen &&
