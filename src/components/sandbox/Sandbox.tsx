...
function Sandbox({ sandboxImage, memLimit, numCPU }: props) {
  ...
  const okButtonClass =
    "inline-flex justify-center w-full md:w-32 rounded-md px-4 py-2 bg-green-500 hover:bg-green-600 text-base leading-6 font-medium text-white shadow-sm focus:outline-none focus:border-blue-300 focus:shadow-outline-blue transition ease-in-out duration-150 sm:text-sm sm:leading-5";
  const inputClass =
  ...
  if (sandboxImage.sandboxesId == undefined) {
    instanceFlag = false;
  }
  const OpenSandbox = () => {
    if (instanceFlag) {
  ...
  var meunItems = [
    {
      ...
      onClick: instanceFlag
        ...
        : async () => {
            ...
            openModal();
            const response = await addSandbox(memLimit, numCPU, sandboxImage.id);
            if (response.success) {
              closeModal();
              window.location.reload();
            } else {
      ...
      <div className="flex flex-row space-x-4 items-start">
        <div className="w-1/12 mt-4">
          <span className="relative flex h-3 w-3">
            ...
            <span
              className={`${baseClass} ${
                instanceFlag ? activeClass : inactiveClass
              }`}
            ></span>
          </span>
        </div>
        <div className="w-full">
          <div className="text-[#578CB5] font-semibold text-sm">
            {sandboxImage.title}
          </div>
          {/* <div>
