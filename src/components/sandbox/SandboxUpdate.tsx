...
const SandboxUpdate = React.forwardRef(({memLimit, numCPU,sandbox,  closeModal }: SandboxUpdateProps, ref) => {


    const [sandboxName, setSandboxName] = useState(sandbox.title)
            ...
            return (
                ...
                <div ref={ref} className={dialogClass}>
                    ...
                    <div className="py-2 m-auto ">
                        A new container is prepared, please click the following link and set up the sandbox. After finished the setting, please press the finish button to save the sandbox image
                    </div>
                    ...
                    <div className={buttonsClass}>
                        <button
                            onClick={async () => {
                                ...
                                nextStep()
                                const response = await updateSandboxImage(sandbox.id, sandboxName, description, containerID, userId)//non-existent template id
            ...
            return (
                ...
                <div ref={ref} className={dialogClass}>
                    ...
                    <input className={inputClass}
                        ...
                        value={sandboxName}
                        onChange={(e) => setSandboxName(e.target.value)}></input>
                    ...
                    <div className={buttonsClass}>
                        <button
                            onClick={async () => {
                                nextStep()
                                const response = await addTempContainer( memLimit, numCPU, sandbox.imageId, sub,  "root")//non-existent template id
                                console.log(response)
                                if (response.success) {
                                    setContainerID(response.tempContainerId)
                                }
                        ...
                        <button onClick={async () => {
                            const response = await updateSandboxImage(sandbox.id, sandboxName, description, "", userId)//non-existent template id
                            console.log(response)
                            if (response.success == true || response.success == false) {
                                window.location.reload();
                            }
