...
const SandboxCreate = React.forwardRef(({ memLimit, numCPU , closeModal }: SandboxCreateProps, ref) => {
            ...
            return (
                ...
                <div ref={ref} className={dialogClass}>
                    ...
                    <div className="py-2 text-gray-600 dark:text-gray-300">
                        Preparing the base IDE Sanbox with base environment, please wait...
                    </div>
                    <Loader></Loader>
                </div>
            )
                    ...
                    <div className={buttonsClass}>
                        <button
                            ...
                            onClick={async () => {
                                ...
                                nextStep()
                                const response = await addSandboxImage( description,  containerID, sandboxName,userId)
                                if (response.success) {
                ...
                <div ref={ref} className={dialogClass}>
                    ...
                    <div className={buttonsClass}>
                        <button
                            onClick={async () => {
                                nextStep()
                                const response = await addTempContainer( memLimit, numCPU,rootImage,sub ,"root")//non-existent template id
                                if (response.success) {
