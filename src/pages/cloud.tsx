...
import { DragLayerMonitorProps, NodeModel, Tree, getDescendants, DropOptions, useOpenIdsHelper } from "@minoru/react-dnd-treeview";
...
async function updateTree(tree: GoogleFolder, id: string, prevPath: string, sub: string, forceUpdate?: boolean): Promise<GoogleFolder> {
    return await new Promise(async (resolve, reject) => {
        var treeCopy = tree
        if (treeCopy.id == id) {
            treeCopy.closed = !treeCopy.closed
            if (treeCopy.closed == false) {
                if ((treeCopy.children.length == 0 && treeCopy.files.length == 0) || forceUpdate) {
                    // fetch data
...
const googleFolderConvert = (id: string, name: string, path: string, closed: boolean, response: any): GoogleFolder => {
    const { loadedFiles: { files, folders } } = response
...
function treeToUITree(tree: DirectoryTree, parent: string | number): (NodeModel<CustomData>[]) {
    ...
    if (tree != undefined) {
        ...
        if (children != undefined) {
            ...
            for (let t of children)
                uitree = uitree.concat(treeToUITree(t, 0))
            return uitree
        }
    }
    return undefined
...
const convertMTree = (tree: GoogleFolder, parent: string | number): NodeModel<CustomData>[] => {
    if (tree != undefined) {
        let children = tree.children
        let uitree: NodeModel<CustomData>[] = []
        if (children != undefined) {
...
const newTree = (oldTree: GoogleFolder, newItem: GoogleFolder): GoogleFolder => {
    // find the corresponding item in the old tree and replace it 
    if (oldTree.id == newItem.id)
        return newItem
    let temp: GoogleFolder = {
        id: oldTree.id,
        name: oldTree.name,
...
export default function Cloud() {
    ...
    // console.log(progressRef1.current)
    ...
    return (
        <div className="grid grid-cols-2 text-black max-h-screen h-full min-h-fit gap-6 p-10 bottom-0 w-full whitespace-nowrap">
            ...
            <div className=" flex flex-col ">
                <p className="text-gray-600 font-bold text-xl dark:text-gray-300 h-fit">Personal Volume</p>
                <FTree
                    ref={ref1}
                    fastDropCallback={() => {
                        progressRef1.current = status.preparing
                    }}
                    handleDropzone={async (acceptedFiles, fileRejections, event, node: NodeModel<CustomData>) => {
                        ...
                        const target = node ? (node?.droppable ? node.data.filePath : dirname(node.data.filePath)) : `/volumes/${userId}/persist`
                        formData.append("filePath", target)
                        let data = await uploadFiles(userId, formData);
                        console.log(data)
                        progressRef1.current = ""
                        if (data.success)
                            return undefined // some problem with the api
                        else {
                            alert(data.message)
                            return undefined
                        }
                    }}
                    handleMoveWithinTree={async (treeData, { dragSourceId, dropTargetId, dragSource, dropTarget }) => {
                        progressRef1.current = status.transfering
                        const target = (dropTarget ? dropTarget.data.filePath : `/volumes/${userId}/persist`) + "/" + path.basename(dragSource.data.filePath)
                        const data = await moveFile(userId, dragSource.data.filePath, target)
                        progressRef1.current = ""
                        console.log(data)
                        if (data.success)
                            return convertTree(data.tree)
                        else
                    ...
                    handleMoveFromAnotherTree={async (treeData, dropTarget) => {
                        ...
                        await googleAPI.downloadFiles(sub, draggingRef.current.node.id, draggingRef.current.node.text, target, draggingRef.current.node.data.fileType)
                        const data = await listFolders(userId)
                        progressRef1.current = ""
                        if (data.success)
                            return convertTree(data.root)
                        else
                            alert(JSON.stringify(data.message))
                    }}
                    handleCloseAll={() => { ref1.current.closeAll() }}
                    handleOpenAll={() => { ref1.current.openAll() }}
                    ...
                    createFolder={async function (node: NodeModel<CustomData>, name: string): Promise<NodeModel<CustomData>[]> {
                        const target = `${node.droppable ? node.data.filePath : path.dirname(node.data.filePath)}/${name}`
                        progressRef1.current = status.loading
                    ...
                    remove={async function (node: NodeModel<CustomData>): Promise<NodeModel<CustomData>[]> {
                        progressRef1.current = status.loading
                    ...
                    edit={async function (node: NodeModel<CustomData>, newName: string): Promise<NodeModel<CustomData>[]> {
                        progressRef1.current = status.loading
                        const data = await moveFile(userId, node.data.filePath, `${path.dirname(node.data.filePath)}/${newName}`)
                        progressRef1.current = ""
                        if (data.success)
                            return convertTree(data.tree)
                        else
                    ...
                    download={async function (node: NodeModel<CustomData>): Promise<void> {
                        progressRef1.current = status.loading
                        const data = await downloadFileToUser(userId, node.data.filePath, node.droppable)
                        if (data != undefined) {
                            // console.log(res)
                    ...
                    canDrop={
                        (tree, { dragSourceId, dragSource, dropTarget, dropTargetId }) => {
                            if (dropTarget == undefined || dragSource == undefined) return true
                            if (dropTarget.data.filePath.includes(dragSource.data.filePath)) return false;
                            if (dragSource.id == dropTarget.id) return false
                            if (dragSource.parent == dropTarget.parent && !dropTarget.droppable) return false
            ...
            {/* cloud  */}
            <div className="flex flex-col">
                <p className="text-gray-600 font-bold text-xl dark:text-gray-300 h-fit">Cloud Volume (Google Drive)</p>
                {
                    !filesRef.current ? <button
                        ...
                        <FTree
                            ...
                            handleMoveFromAnotherTree={async (treeData, dropTarget) => {
                                ...
                                if (dropTarget.droppable) {
                                    ...
                                    const response = await googleAPI.uploadFiles(sub, draggingRef.current.node.data.filePath, dropTarget.id, draggingRef.current.node.data.fileType);
                                    if (!response.success)
                                        return treeData
                                    ...
                                    const temp = googleFolderConvert(String(dropTarget.id), dropTarget.text, dropTarget.data.filePath, false, response2)
                                    console.log(temp)
                                    filesRef.current = newTree(filesRef.current, temp)
                                } else {
                                    console.log("not droppable")
                                    const response = await googleAPI.uploadFiles(sub, draggingRef.current.node.data.filePath, dropTarget.parent, draggingRef.current.node.data.fileType);
                                    if (!response.success)
                                        return treeData
                                    // console.log(files, String(dropTarget.parent), dropTarget.data.filePath)
                                    const response2 = await googleAPI.expandFolder(dropTarget.parent, sub)
                                    const temp = googleFolderConvert(String(dropTarget.parent), dropTarget.text, dropTarget.data.filePath, false, response2);
                                    console.log(temp)
                                    filesRef.current = newTree(filesRef.current, temp)
                            ...
                            onClick={async (treeData, node) => {
                                if (node.droppable && getFolderById(filesRef.current, String(node.id)).children.length == 0) {
                                    progressRef2.current = status.loading
                                    const response = await googleAPI.expandFolder(node.id, sub)
                                    const newItems = googleFolderConvert(String(node.id), node.text, node.data.filePath, false, response)
                            ...
                            canDrop={(tree, { dragSourceId, dragSource, dropTarget, dropTargetId }) => {
